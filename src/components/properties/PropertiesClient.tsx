...
export default function PropertiesClient({
  ...
  const maxPrice = Math.max(
    ...properties.map((p) => p.startingPrice ?? p.price ?? 0),
    0,
  );
  ...
  const filteredProperties = useMemo(() => {
    return properties.filter((p) => {
      const searchMatch =
        searchTerm.length > 2
          ? p.title.toLowerCase().includes(searchTerm.toLowerCase()) ||
            p.locationLine.toLowerCase().includes(searchTerm.toLowerCase())
          : true;
      ...
      const priceMatch =
        price >= finalPriceRange[0] && price <= finalPriceRange[1];

      // Updated Status Logic
      const statusMatch =
        status === "all" ||
        (status === "Sale & New"
          ? p.status === "For Sale" || p.status === "New Launch"
          : p.status === status);
    ...
    <div className="space-y-6">
      <div>
        ...
        >
          ...
          <div className="flex items-center space-x-2">
            <RadioGroupItem value="Sale & New" id="r-sale-new" />
            <Label htmlFor="r-sale-new">Sale / New</Label>
          </div>
          <div className="flex items-center space-x-2">
    ...
    <div>
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <aside className="hidden lg:block lg:col-span-1 bg-card p-6 rounded-lg shadow-sm h-fit sticky top-24 border animate-in fade-in-0 slide-in-from-left-12 duration-500">
          <h2 className="text-2xl font-semibold mb-6 font-headline text-accent">
            Filters
          </h2>
          <Filters />
        </aside>
              ...
              <Sheet>
                ...
                <SheetContent side="left" className="w-[300px] sm:w-[400px]">
                  <SheetHeader className="mb-6">
                    <SheetTitle className="text-2xl font-semibold font-headline text-accent">
                      Filters
                    </SheetTitle>
                  </SheetHeader>
                  <Filters />
